...
data_root = '/home/harish/l5kit/l5kit/train/'
...
def generate_data(pra_file_path_list, pra_is_train=True):
	...
	"""
	if pra_is_train:
		save_path = "train_data/train_data_{0}.pkl".format(scene)
	else:
...
if __name__ == '__main__':
	train_path = '/home/harish/l5kit/l5kit/train_new'
	test_path = 'ApolloScape/prediction_test'
	train_file_path_list = sorted(glob.glob(os.path.join(train_path,'*.txt')))
	test_file_path_list = sorted(glob.glob(os.path.join(test_path,'*.txt')))

	print('Generating Training Data.')
	for i in range (1,1000):
		path = train_path+"/scene_"+str(i)
		train_file_path_list = sorted(glob.glob(os.path.join(path,'*.txt')))
		try:
			generate_data(train_file_path_list, i,pra_is_train=True)
		except:
			pass

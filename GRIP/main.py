...
def my_save_model(pra_model, pra_epoch):
	path = '{}/model_epoch.pt'.format(work_dir, pra_epoch)
	torch.save(
		{
...
def data_loader(pra_path, pra_batch_size=16, pra_shuffle=False, pra_drop_last=False, train_val_test='train'):
...
def run_trainval(pra_model, pra_traindata_path):
	loader_train = data_loader(pra_traindata_path, pra_batch_size=batch_size_train, pra_shuffle=False, pra_drop_last=True, train_val_test='train')
...
if __name__ == '__main__':
	graph_args={'max_hop':2, 'num_node':150}
	model = Model(in_channels=4, graph_args=graph_args, edge_importance_weighting=True)
	model.to(dev)

	# train and evaluate model
	run_trainval(model, pra_traindata_path='/home/harish/l5kit/l5kit/train_temp')
